	...
	try:
		...
		match req.platform:
			...
			case schemas.Platform.minecraft:
				platform = MinecraftUsers

		# required values
		platform_id = req.platform_id
		message_length = req.message_length
		current_timestamp = time.time()
		message_bonus = get.discord_msg_bonus(db)
		tax_rate = get.discord_tax_rate(db)

		# get unid or None
		row = get.platform_row(platform, platform_id, db)

		if not row: # new user
			new_user = True
			unid = new.platform_user(platform, platform_id, db)
			time_gap = 10
			message_count = 1
		else:
			unid = row.unid
		...
		db.query(platform).filter(
			...
			{ "message_count": platform.message_count +1, "last_message": current_timestamp }
		)
		...
		# give coin
		new.eval_coin(unid, total_value, to_admin, db)
