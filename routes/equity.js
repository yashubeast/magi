...
router.get('/balance', async (req, res) => {
	...
	try {
		...
		const auuid = await get.user(user_id, db)
...
// minecraft routes
router.post('/evalmc', async (req, res) => {
	const parsed = schema.EvalMC.safeParse(req.body)
	if (!parsed.success) {
		return res.status(400).json({ result: parsed.error.format() })
	}

	try {
		const { user_id, message_length, timestamp } = parsed.data
		const result = await func.evalMinecraft( user_id, message_length, timestamp, db )
		res.status(200).json({ result })
...
router.post('/balancemc', async (req, res) => {
	const parsed = schema.Balance.safeParse(req.body)
	if (!parsed.success) {
		return res.status(400).json({ result: parsed.error.format() })
	}
	...
	try {
		const { user_id } = parsed.data
		const auuid = await get.userMinecraft(user_id, db)
		const result = await get.balance(auuid, db)
		res.status(200).json({ result })
		console.log('balance hit')
	} catch (err) {
...
router.post('/paymc', async (req, res) => {
	const parsed = schema.TransferCoin.safeParse(req.body)
	if (!parsed.success) {
		return res.status(400).json({ result: parsed.error.format() })
	}
	...
	try {
		const { sender_id, receiver_id, amount } = parsed.data
		const result = await func.transferCoin( sender_id, receiver_id, amount, db, "minecraft" )
		res.status(200).json({ result })
		console.log('200')
	} catch (err) {
		res.status(err.status || 404).json({ result: err.message })
